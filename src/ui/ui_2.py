...
INDEX_DESC = """
- **Need(취약 지수)**: 자살률·우울·스트레스 등 위험 요인을 표준화해 종합한 상대 지수  
- **Supply(인프라 지수)**: 의료·복지·문화·체육 인프라 수준을 표준화한 상대 지수  
- **Gap(격차 지수)** = Need − Supply  
  (＋: 수요 대비 공급 부족 / −: 상대적 공급 여유)
"""
...
if st.session_state.current_page == "home":
    ...
    cards = [
        ...
        ("gap", "📊", "수요-공급 격차 분석", "취약도와 인프라 수준의 균형 분석"),
...
# 7. 서브 페이지
    ...
    if geo_data is None or infra_data is None:
        ...
        if page == 'mhvi':
            ...
            st.info("""
💡 **이 지도는 무엇을 보여주나요?**  
지도의 색상은 각 자치구의 **정신건강 관심도 수준**을 나타냅니다.  
...
- 🔵 **D (고위험 대응형)**: 취약도는 매우 높지만 인프라는 갖춰진 지역
            """)
            ...
            st.info("""
💡 **Need/Supply 역설: 높은 위험도 × 높은 인프라 = 사각지대?**

일반적으로 **위험도가 높으면 인프라도 많아야** 정상이지만,  
실제로는 **위험도와 인프라가 동시에 높은 지역**이 존재합니다.

이는 다음과 같은 **정책 사각지대**를 의미할 수 있습니다:
- 📍 **인프라는 있지만 실제 필요에 못 미침** (양은 많지만 질이 부족)
- 📍 **인프라가 있어도 접근성/활용도가 낮음** (있지만 닿지 않음)
- 📍 **구조적으로 해결되지 않는 지역** (지속적 관리 필요)

👉 이런 지역은 **단순 인프라 확충이 아닌, 맞춤형 정책 개입**이 필요합니다.

---
🚫 **정책 사각지대의 핵심 (Supply-Need Paradox)**  
**인프라(Supply) 공급 수준이 높음에도 불구하고 정신건강 위험도(Need)가 여전히 높은 지역**은 투입된 예산과 자원이 효율적으로 활용되지 못하고 있음을 시사합니다.
즉, **자원 낭비가 의심되거나 기존 정책의 전면 재검토가 필요한 '비효율적 고위험 지역'**으로 해석해야 합니다.
            """)
            ...
            if os.path.exists(RANK_PATH) and os.path.exists(SHAP_PATH):
                ...
                with c1:
                    st.markdown("**1️⃣ 정책 사각지대 의심 지역 (상위 10개)**")
